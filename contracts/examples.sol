...
contract examples {
	...
	function examples() {
		address[] selves = new address[](2);
		selves[0] = this;
		selves[1] = this;

		unanimousContract = new UnanimousConsent(selves);
	...
	function fullOnChainPaymentChannel() returns(bool) {
		...
		actionsToSend[0] = Action(_target, 0, calculateTxData("foo(uint256)", _args), true); //instantiate lockAdjudicator
		actionsToSend[1] = Action(_target, 0, calculateTxData("submit(bytes32[])", _args), true); //instantiate payment channel adjudicator
		actionsToSend[2] = Action(_target, 0, calculateTxData("submit(bytes32[])", _args), true); //update the state of the payment adjudicator
		actionsToSend[3] = Action(_target, 10000, calculateTxData("submit(bytes32[])", _args), true); //make the payment based on the finalised state
	...
	function calculateTxData(string _functionSignature, bytes32[] _args) returns(bytes4) {
		//todo: finish properly forming the actual thing
		//note: this will be hard to do without some assembly...
